...
> **작성 규칙 원본(온톨로지)도 이 리포에 함께 있습니다.** 2026년 9월부터 규칙을 모두 공개합니다. 사람이 읽기 쉽게 풀어 쓴 규칙 카드는 [규칙마당](https://rules.artifact-intelligence.app)에서 볼 수 있습니다. 설치본은 지금도 실행 중에 필요한 규칙 조각만 정책 서버에서 받아 옵니다. 판정·조립·검사·변환 코드는 사용자 문서를 서버로 보내지 않으려고 설치한 컴퓨터에서 돕니다.
...
- **온톨로지 지식**은 이 리포에도 공개돼 있고, 설치본은 `지식()`으로 정책 서버에서 필요한 조각만 조회합니다.
...
└─ artifact-intelligence/            ── 본체 (웹앱 + 플러그인 + MCP 가 공유) ──
   ...
   ├─ ontology/                      작성 규칙 원본(ontology.json)·실측 타이포·파급표·편집기 프로파일·검토본
   ├─ references/                    규칙 해설 문서(압축·문체·서식·보고 유형)
   ├─ rules/                         규칙마당 공개 페이지·규칙 카드·예시 그림·경량 스킬 zip
   │  └─ build.py                    카드·예시 그림·경량 스킬을 다시 만드는 스크립트
   ├─ research/rules-commons/        규칙마당 도구(예시 그림 렌더러·주석·경량 스킬 빌더)
